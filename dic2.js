...
function data(result, dic_word) {
    if (result.title) {
        ...
        let definitions = result[0].meaning[0];
        ...
        document.querySelector(".dic_meaning span").innerText = definitions.firstOccurence;
        document.querySelector(".dic_example span").innerText = definitions.example;
        ...
        if(definitions.relatedLinks[0] == undefined){ //if there is no synonym then hide the synonym div
            ...
            for (let i = 0; i < 5; i++) { //getting only 5 relatedLinks out of many
                let tag = `<span onclick=search('${definitions.relatedLinks[i]}')>${definitions.relatedLinks[i]},</span>`; 
